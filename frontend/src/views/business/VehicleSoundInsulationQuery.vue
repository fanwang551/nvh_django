          ...
          <el-col :span="16">
            <el-form-item label="对比车型" required>
              ...
              >
                ...
                <el-option
                    v-for="vehicle in filteredVehicleModelOptions"
                    :key="vehicle.id"
                    :label="vehicle.vehicle_model_name"
                    :value="vehicle.id"
                />
                <el-option
                    v-if="!filteredVehicleModelOptions.length"
                    :value="null"
                    disabled
                >
...
// 渲染ECharts图表
const renderChart = () => {
  console.log('开始渲染图表，容器存在:', !!chartRef.value, '数据长度:', store.chartData.length)
