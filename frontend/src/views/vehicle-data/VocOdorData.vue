        ...
        <el-row :gutter="20" style="margin-top: 20px;">
          ...
          <el-col :span="6">
            <el-form-item label="样品编号">
              ...
              <el-select
                v-if="store.searchCriteria.project_names.length > 0"
...
// 监听项目名称变化，联动加载样品编号选项
watch(
  () => store.searchCriteria.project_names,
  async (newProjectNames) => {
    // 当项目名称变化时，清空已选的样品编号（多选模式使用空数组）
    store.searchCriteria.sample_no = []
    
    // 如果有选中的项目，则加载对应的样品编号选项
    if (newProjectNames && newProjectNames.length > 0) {
      try {
        await store.fetchSampleNoOptions(newProjectNames)
      } catch (error) {
