# 试验流程数字化（NVH Task）提示词优化版（可执行）

> 目的：把 `frontend/试验流程数字化需求最终版.md`（需求说明）和 `frontend/试验流程数字化任务清单.md`（执行清单）优化成**更适合“代码 Agent”直接执行**的提示词。

---

## 0. 可执行性结论（对现有两份文档的判断）

结论：**整体可执行**，并且信息密度高、约束明确，已经具备“让 Agent 直接开工”的基础。

主要可优化点（影响 Agent 执行效率/一致性）：
1. **“目标/交付物/非目标”缺少统一声明**：两份文档分别讲需求与清单，但没有一个“总指挥段落”把范围边界一次说清（尤其是“允许改哪些文件”“不提供 migration 文件”的执行方式）。
2. **“必须/建议/可选”标记不够一致**：Agent 容易把“建议”当“必须”实现，或遗漏“强制落地”的体验点（如统一抽屉 Tabs + 闭环提示条的一键定位）。
4. **接口合同（endpoint/参数/返回字段）分散**：可读性强，但执行时最好把“后端 API 列表”集中成一页，减少来回翻。


---

## 1. 提示词编写原则（让 Agent 能理解并执行）

1. **先给“总目标 + 非目标 + 交付物”**：一句话目标、明确不做什么、列出最终必须新增/修改的文件与功能点。
2. **把“硬约束”集中列出**：例如软删除、闭环刷新触发点、状态常量、筛选字段、白名单权限、禁止提供 migration 文件等。
3. **把“术语与对象关系”一次定义清楚**：MainRecord / EntryExit / TestInfo / DocApproval / TestProcessAttachment 的关系、共享规则、闭环规则。
4. **把“接口合同”集中成清单**：每个 endpoint 的 method/path、关键入参、关键返回字段、错误码与错误信息形态（至少后端要返回 400 可读错误）。
5. **把“触发规则/时机”写成可执行规则**：例如 `is_closed` 只能在 submit/unsubmit/解绑 entry_exit 时刷新并落库，列表只读字段不实时算。
6. **把“可选项”明确标记为可选，并写默认实现**：避免 Agent 为追求完美过度扩展范围。
7. **每个任务块写清：改动范围 + 完成条件**：让 Agent 能自检“这一步是否完成”。
8. **明确“遇到不确定时怎么做”**：缺信息 → 提问；有冲突 → 以硬约束优先；需要参考现有写法 → 指定参考文件路径。
9. **提供最小验证路径**：给出最少的可执行检查命令（并写清是否允许生成临时文件、如何清理）。
10. **保持可复制粘贴**：把最终要给 Agent 的内容整理成两个可直接复制的块（主提示词 + 清单），避免夹杂太多解释性文字。

---

## 2. 优化版提示词 A（主提示词：需求 + 约束 + 合同）

> 用法：把本节整段复制给 Agent，作为“主提示词”。然后再贴第 3 节的“执行清单”。
...
### 2.2 非目标（明确不做）
- 不需要编写自动化测试。
- **最终提交中不包含 migration 文件**（但代码必须可 `makemigrations` 通过）。
...
- serializer 需做到：
  ...
  - 写：允许前端传“字符串相对路径/URL/空”时不崩溃（按项目现有上传策略做兼容）
...
## 3. 优化版提示词 B（执行清单：按步落地）

> 用法：把本节整段复制给 Agent，作为“可勾选 checklist”。每一步都应写清改动文件与完成条件。
...
1. 先贴：第 2 节（主提示词 A）
2. 再贴：第 3 节（执行清单 B）
3. 最后补充：你希望 Agent 的输出形式（例如“只输出改动文件列表 + 关键说明”或“直接落地代码并自检”）
