...
class SoundInsulationCoefficientsSerializer(serializers.ModelSerializer):
    """隔声量系数数据序列化器"""

    class Meta:
        model = SoundInsulationCoefficients
        fields = [
            'id', 'part_name', 'material_composition', 'test_type', 'manufacturer',
            'test_institution', 'thickness', 'weight',
            # 测试值字段
            'test_value_125', 'test_value_160', 'test_value_200', 'test_value_250',
            'test_value_315', 'test_value_400', 'test_value_500', 'test_value_630',
...
class SoundInsulationCoefficientQuerySerializer(serializers.Serializer):
    """隔声量系数查询参数序列化器"""
    test_type = serializers.CharField(required=False, help_text='测试类型')
    part_name = serializers.CharField(required=False, help_text='零件名称')
    material_composition = serializers.CharField(required=False, help_text='材料组成')
    weight = serializers.DecimalField(max_digits=8, decimal_places=2, required=False, help_text='克重(g/m²)')
...
class TestTypeOptionSerializer(serializers.Serializer):
    """测试类型选项序列化器"""
    value = serializers.CharField()
    label = serializers.CharField()


class InsulationPartNameOptionSerializer(serializers.Serializer):
    """隔声量零件名称选项序列化器"""
    value = serializers.CharField()
    label = serializers.CharField()


class InsulationMaterialCompositionOptionSerializer(serializers.Serializer):
    """隔声量材料组成选项序列化器"""
    value = serializers.CharField()
    label = serializers.CharField()
    part_name = serializers.CharField()
...
class InsulationWeightOptionSerializer(serializers.Serializer):
    """隔声量克重选项序列化器"""
    value = serializers.DecimalField(max_digits=8, decimal_places=2)
    label = serializers.CharField()
    part_name = serializers.CharField()
