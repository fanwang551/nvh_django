...
from apps.acoustic_analysis.models import AcousticTestData, DynamicNoiseData
...
class DynamicWorkConditionSerializer(serializers.Serializer):
    vehicle_model_ids = serializers.CharField(required=True, help_text='车型ID列表，逗号分隔')
...
class DynamicMeasurePointSerializer(serializers.Serializer):
    vehicle_model_ids = serializers.CharField(required=True, help_text='车型ID列表，逗号分隔')
    work_conditions = serializers.CharField(required=True, help_text='工况列表，逗号分隔')
...
class DynamicNoiseQuerySerializer(serializers.Serializer):
    vehicle_model_ids = serializers.ListField(
        child=serializers.IntegerField(min_value=1),
        allow_empty=False,
    )
...
class DynamicNoiseTableSerializer(serializers.ModelSerializer):
    vehicle_model_name = serializers.CharField(read_only=True)
    work_condition = serializers.CharField(source='condition_measure_point.work_condition', read_only=True)
    measure_point = serializers.CharField(source='condition_measure_point.measure_point', read_only=True)

    class Meta:
        model = DynamicNoiseData
        fields = (
            'id',
            'vehicle_model_id',
            'vehicle_model_name',
